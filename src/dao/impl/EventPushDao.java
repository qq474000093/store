...
public class EventPushDao implements dao.EventPushService  {
	...
	public int GetUserCardPush(EventPush eventpush) {
		String sql="insert into cardpush(MsgType,Event,Cardid,UserCardCode,openid,OuterStr) values(?,?,?,?,?,?) ;";
		return DBUtil.update(sql, eventpush.getMsgType(),eventpush.getEvent(),eventpush.getCardId(),eventpush.getUserCardCode(),eventpush.getFromUserName(),eventpush.getOuterStr());
